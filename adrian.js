...
( Q.S ${res.result.data.surah.name.transliteration.id} : ${res.result.data.number.inSurah} )`
		...
		case 'brainly':{
			...
			case 'mcstatus': {
  const ip = 'u4.justshop.cloud';
  const port = 19129;

  newReply('Sedang memeriksa status server...');

  const apiUrl = `https://api.mcstatus.io/v2/status/bedrock/${ip}:${port}`;
  ...
  axios
    ...
    .then((response) => {
      ...
      if (data.online) {
        const { hostname, version, players, description } = data;
        const playerCount = players.online || 0;
        const maxPlayers = players.max || 0;

        let status = `Server: ${hostname}\n`;
        status += `Versi: ${version}\n`;
        status += `Pemain: ${playerCount}/${maxPlayers}\n`;
        status += `Deskripsi: ${description}`;

        newReply(status);
      } else {
        newReply('Server sedang offline.');
      }
    })
    .catch((error) => {
      console.error(error);
      newReply('Terjadi kesalahan saat memeriksa status server.');
    });
  break;
}
